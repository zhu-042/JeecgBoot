...
navigationBarTitleText: '智能手表产品信息表',
},
}
</route>
<template>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['productCode']"
                  :label="get4Label('产品编码')"
                  name='productCode'
                  prop='productCode'
                  placeholder="请选择产品编码"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['productName']"
                  :label="get4Label('产品名称')"
                  name='productName'
                  prop='productName'
                  placeholder="请选择产品名称"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('SmartWatchProductList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/org/jeecg/modules/smartwatch/smartWatchProduct/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/org/jeecg/modules/smartwatch/smartWatchProduct/edit':'/org/jeecg/modules/smartwatch/smartWatchProduct/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
