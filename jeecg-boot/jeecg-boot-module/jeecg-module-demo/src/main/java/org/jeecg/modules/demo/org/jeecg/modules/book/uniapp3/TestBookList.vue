...
<template>
  <PageLayout navTitle="图书管理"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TestBookData';
defineOptions({
  name: 'TestBookList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/org/jeecg/modules/book/testBook/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/org/jeecg/modules/book/testBook/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
