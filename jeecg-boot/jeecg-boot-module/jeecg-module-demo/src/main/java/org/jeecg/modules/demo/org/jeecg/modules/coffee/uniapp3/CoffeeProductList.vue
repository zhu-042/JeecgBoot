...
<template>
  <PageLayout navTitle="咖啡产品表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CoffeeProductData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/org/jeecg/modules/coffee/coffeeProduct/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/org/jeecg/modules/coffee/coffeeProduct/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
