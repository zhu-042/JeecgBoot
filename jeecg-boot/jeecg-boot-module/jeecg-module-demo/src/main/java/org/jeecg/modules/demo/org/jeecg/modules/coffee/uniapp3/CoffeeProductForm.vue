...
const dataId = ref('')
const backRouteName = ref('CoffeeProductList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/org/jeecg/modules/coffee/coffeeProduct/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/org/jeecg/modules/coffee/coffeeProduct/edit':'/org/jeecg/modules/coffee/coffeeProduct/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
