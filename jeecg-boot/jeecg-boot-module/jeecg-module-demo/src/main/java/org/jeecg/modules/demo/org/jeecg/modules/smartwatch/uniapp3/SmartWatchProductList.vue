...
<template>
  <PageLayout navTitle="智能手表产品信息表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './SmartWatchProductData';
defineOptions({
  name: 'SmartWatchProductList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/org/jeecg/modules/smartwatch/smartWatchProduct/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/org/jeecg/modules/smartwatch/smartWatchProduct/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
