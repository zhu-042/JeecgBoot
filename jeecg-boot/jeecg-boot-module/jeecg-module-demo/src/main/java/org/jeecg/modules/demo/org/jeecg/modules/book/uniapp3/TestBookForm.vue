    ...
    <scroll-view class="scrollArea" scroll-y>
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('出版日期')"
                  labelWidth="100px"
                  type="date"
                  name='publishDate'
                  v-model:value="myFormData['publishDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('TestBookList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/org/jeecg/modules/book/testBook/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/org/jeecg/modules/book/testBook/edit':'/org/jeecg/modules/book/testBook/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
